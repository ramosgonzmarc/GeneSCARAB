...
HermansRasson2T <- function(sample) {
    ...
    for (i in seq_len(n)) {
        for (j in seq_len(n)) {
            total <- total + abs(abs(sample[i] - sample[j]) - pi) -
                (pi / 2)
            total <- total - (2.895 * (abs(sin(
                sample[i] - sample[j]
            )) - (2 / pi)))
        }
    }
...
#'
#' Helper function that calculates the p-value of an adaptation of the
#' Hermans-Rasson test for grouped data (discrete phases) as
#' implemented in the article "Grouped circular data in biology:
#' advice for effectively implementing statistical procedures"
#' https://doi.org/10.1007/s00265-020-02881-6.
#'
#'
#' @param sample Data points in radians
...
HermansRasson2PGroupedRad <- function(sample, m, iter = 9999) {
    ...
    for (f in seq_len(univals)) {
        data1 <- circular::rcircularuniform(
            n,
            control.circular = list(units = "radians")
        )
        data1 <- trunc(data1 * m / (2 * pi))
        data1 <- data1 * 2 * pi / m
        errorsamp <- circular::rvonmises(n, 0, k, control.circular = list(
            units =
                "radians"
        ))
...
#'
#' Helper function that calculates the critical value for
#' Rao's spacing test as implemented in the article "Grouped
#' circular data in biology: advice for effectively implementing
#' statistical procedures" https://doi.org/10.1007/s00265-020-02881-6.
#'
#' @param sample Data points in radians
#'
#' @returns The critical value for the test
...
#' for ungrouped data (continuous phases) as implemented
#' in the article "Circular statistics meets practical
#' limitations: a simulation-based Rao’s spacing
#' test for non-continuous data"
...
#'
#' Helper function that calculates the p-value of an
#' adaptation of the Rao's spacing test for grouped data
#' (discrete phases) as implemented in the article "Grouped
#' circular data in biology: advice for effectively
#' implementing statistical procedures"
#' https://doi.org/10.1007/s00265-020-02881-6.
#'
#' @param sample Data points in radians
#' @param m Number of bins
...
#'
#' Helper function that calculates the p-value of an adaptation
#' of the Kuiper test for grouped data (discrete phases) as
#' implemented in the article "Grouped circular data in
#' biology: advice for effectively
#' implementing statistical procedures"
#'  https://doi.org/10.1007/s00265-020-02881-6.
#'
#' @param sample Data points in radians
...
#' term based on a species annotation package. This includes
#' all ancestor GO terms of the selected ones.
#'
...
#'
#' @returns A list linking each GO term with the genes
#' that share that annotation
#' @export
#'
#' @examples
...
#' )
#'
create_gene_list_go <- function(go_vector = c("all"),
                                org.package,
                                go_column,
                                id_column) {
...
#'
#' Function to create a list of genes associated with each KEGG
#' pathway based on a species annotation package with KO terms
#' derived from KEGGREST, KAAS or similar tools. It also allows
...
#'
#' @returns A list linking each KEGG pathway with the genes
#' that share that annotation
#' @export
#'
#' @examples
...
create_gene_list_kegg <- function(ko_vector = c("all"),
    ...
    kos_pathways_clean <- lapply(kos_pathways_clean, function(x) {
        vapply(strsplit(x, ":"), function(y) {
            y[[2]]
        }, character(1))
    })
...
#'
#' Function that takes a list of genes associated with
#' specific biological processes (GO, KEGG) or gene sets
#' and a general phase table, and returns a list consisting
#' of the phase tables specific to each set.
#'
...
#'
#' Function that takes a vector of gene phases (continuous
...
#' @param force.hr.th Significance level to execute HR test if
#' Rayleigh and Kuiper p-values are higher
#' @param rao.on.large.sets.th Threshold indicating the maximum
#' size of a set to perform Rao test on
#' @param iter.rao Number of iterations for Rao test
#' @param force.rao.th Significance level to execute Rao test if
#' Rayleigh, Kuiper and HR p-values are higher
#'
...
#' Creation of the circular table and deviation from the
#' uniform distribution for a single set of grouped data
#'
#' Function that takes a vector of gene phases (discrete
#' measurements or non-parametric estimates)
#' and returns statistical measures of their circular
...
#' @param force.rao.th Significance level to execute Rao test if
#' Rayleigh, Kuiper and HR p-values are higher
#' @param kuiper.on.large.sets.th Threshold indicating the maximum
#' size of a set to perform Kuiper test on
#' @param iter.kuiper Number of iterations for Kuiper test
#' @param force.kuiper.th Significance level to execute Kuiper
#' test if Rayleigh p-value is higher
#'
#' @returns A data.frame indicating number of phases used for
#' computation (n); mean, rho, variance, first, second and third
...
#' Creation of the circular table and deviation
#' from the uniform distribution for a phases list
#'
#' Function that takes a list of continuous phases
...
#' @param hr.on.large.sets.th Threshold indicating the
#' maximum size of a set to perform HR test on
#' @param iter.hr Number of iterations for HR test
#' @param force.hr.th Significance level to execute HR
#' test if Rayleigh and Kuiper p-values are higher
#' @param rao.on.large.sets.th Threshold indicating the
#' maximum size of a set to perform Rao test on
#' @param iter.rao Number of iterations for Rao test
#' @param force.rao.th Significance level to execute Rao
#' test if Rayleigh, Kuiper and HR p-values are higher
#'
#' @returns A data.frame indicating, for each process or
#' gene set: number of phases used for computation (n); mean,
#'  rho, variance, first, second and third quantiles of the
#'  circular distribution in hours; p-values and adjusted
#'   p-values for each of the four tests
#' @export
#'
#' @examples
...
#' )
#'
complete_circular_table <- function(phase.list,
                                    hr.on.large.sets.th = 400,
                                    iter.hr = 999,
                                    force.hr.th = 0.05,
                                    rao.on.large.sets.th = 400,
...
#'
#' @param phase.list Phases list in the same format as the output
#' of gene_list_to_phases
#' @param n_bins Number of bins
#' @param hr.on.large.sets.th Threshold indicating the maximum size
#' of a set to perform HR test on
#' @param iter.hr Number of iterations for HR test
#' @param force.hr.th Significance level to execute HR test if Rayleigh
#' and Kuiper p-values are higher
#' @param rao.on.large.sets.th Threshold indicating the maximum size of
#' a set to perform Rao test on
#' @param iter.rao Number of iterations for Rao test
#' @param force.rao.th Significance level to execute Rao test if Rayleigh,
#' Kuiper and HR p-values are higher
#' @param kuiper.on.large.sets.th Threshold indicating the maximum size of
#' a set to perform kuiper test on
#' @param iter.kuiper Number of iterations for kuiper test
#' @param force.kuiper.th Significance level to execute Kuiper test if
#' Rayleigh p-value is higher
#'
#' @returns A data.frame indicating, for each process or gene set:
...
#'
#' Function that compares whether the distributions
#' of specific processes or gene sets differ
#' significantly from the overall phase distribution
#' of the entire study. Significance is calculated
#' based on a MANOVA analysis as described in
#' https://doi.org/10.1186/s40462-022-00323-8.
#'
#' @param phase.list Phases list in the same format
#' as the output of gene_list_to_phases
#' @param total.phase.table Table showing acrophases
#' for the complete gene set under study, stated in hours
#'
#' @returns A data.frame with p-value and adjusted p-value
#'  (null hyphotesis: same distribution as the general one)
#'   for each process or gene set
#' @export
#'
#' @examples
#' library(org.Otauri.eg.db)
...
test_against_gen_dist <- function(phase.list, total.phase.table) {
    ...
    p_values_gen_test <- vapply(phase.list, function(x) {
        angles <- c(gen_dist, x$phase * pi / 12)

        if (length(unique(x$phase)) == 1) {
            angles <- angles + stats::rnorm(length(angles),
                mean = 0, sd = 0.01
            )
        }

        p_values_gen_test <- summary(stats::manova(cbind(
            sin(angles),
            cos(angles)
        ) ~ c(
    ...
    gen_dist_table <- data.frame(
        gen_dist_p_value = p_values_gen_test,
...
#'
#'
#' @param phase.list.1 Phases list for condition 1, in the same
#' format as the output of gene_list_to_phases
#' @param phase.list.2 Phases list for condition 2, in the same
#' format as the output of gene_list_to_phases
#'
#' @returns A data.frame with p-value and adjusted p-value (null
...
test_two_dist <- function(phase.list.1, phase.list.2) {
    if (length(phase.list.1) != length(phase.list.2)) {
        stop("Length of lists differ.")
    }

    if (sum(names(phase.list.1) == names(phase.list.2)) != length(
        phase.list.1
    )) {
    ...
    p_values_diff_test <- mapply(function(x, y) {
        ...
        ~ c(
                rep("first", nrow(x)), rep("second", nrow(y))
            )))$stats[1, 6]
    }, phase.list.1, phase.list.2)

    p_values_diff_bh <- stats::p.adjust(unlist(
        p_values_diff_test
    ), method = "BH")

    p_values_diff_table <- data.frame(
        p_value = unlist(p_values_diff_test),
...
#'
#' Function that measures the contribution to
#' the mean and rho of the circular distribution of a
#' process or gene set for each of its constituent
#' genes by removing them one by one. For sets
#' consisting on more than 100 genes, it takes a
#' random sample of 100 gene phases to normalize
...
#'
#' @returns A list whose elements are tables indicating
#' the difference in circular mean and rho due to removing
#' each gene for each process or gene set, and the rank
#' of each difference
#' @export
#'
#' @examples
...
gene_contribution_to_set <- function(circa_result, phase_list) {
    ...
    {
        new_list <- lapply(seq_len(nrow(phase_list[[x]])), function(y) {
            phase_list[[x]][-y, ]
        })
        new_phases <- lapply(new_list, function(y) {
            circular::circular(y$phase * pi / 12)
        })
        ...
        pos_res <- ifelse(
            as.numeric(new_summaries[, "Mean"]) < 0,
            24 + as.numeric(new_summaries[, "Mean"]),
            as.numeric(new_summaries[, "Mean"])
        )
...
#'
#' Function to determine the most likely number of modes
#' in the presence of f-fold symmetry (1, 2 or 3),
#' cluster genes in f clusters and determine the statistic
...
#'
#' @returns A list containing the most likely mode number,
#' the p-value for that number, the p-values corresponding
#' to each mode number, the circular summary table for each
#'  cluster and the genes contained in each cluster
#' @export
#'
#' @examples
#' library(org.Otauri.eg.db)
...
multimodal_analysis <- function(phase_table) {
    ...
    clusters_focc <- lapply(seq_len(
        max(result_FOCC$cluster)
    ), function(x) {
        phase_table[result_FOCC$cluster == x, ]
    })
    names(clusters_focc) <- paste0("cluster_", seq_len(
        max(result_FOCC$cluster)
    ))

    cluster_radians <- lapply(clusters_focc, function(x) {
        circular::circular(x$phase * pi / 12)
    })
    cluster_summary <- vapply(cluster_radians, function(x) {
        summary(x)
    }, numeric(8))
...
#' Circular boxplot
#'
#' Function to plot a circular boxplot showing the circular
#' distribution of different processes or gene sets
#'
#' @param res.table A circular results table, as the output from
#' complete_circular_table or complete_circular_table_grouped
#' @param color.palette Color palette to use. Available palettes
#'  are those from MetBrewer
...
#' )
#'
circular_boxplot <- function(res.table,
                             color.palette = "Tam",
                             tr.height = 0.1) {
    ...
    {{
        ...
        if ((quan.table$first[i] < quan.table$median[i]) &&
            (quan.table$median[i] < quan.table$third[i])) {
...
#' @param color.palette Color palette to use. Available palettes
#' are those from MetBrewer
#' @param tr.height Height of each track. For more than 8 tracks,
...
circular_dotplot <- function(phases.list,
    ...
    my_transparent <- vapply(my_color, function(x) {
        circlize::add_transparency(x, 0.5)
    }, character(1))
...
#' different processes or gene sets
#'
#' @param phases.list A phase tables list in the same
#' format as the output of gene_list_to_phases
#' @param color.palette Color palette to use. Available
#'  palettes are those from MetBrewer
#' @param tr.height Height of each track. For more than
#' 8 tracks, reduce this parameter
...
circular_histogram <- function(phases.list,
    ...
    for (i in seq_along(counts_list)) {
        ...
        circlize::circos.track(
            ylim = c(0, max(counts_list[[i]])), track.height = tr.height,
...
#' circa_table_genescarab
#'
#' Dataset example with phase information in two
#' different conditions for Ostreococcus tauri
#' from "Multiomics integration unveils photoperiodic
#'  plasticity in the molecular rhythms of
